...
const { socialAssistancesRecipients, meta, loading, error, success } =
    storeToRefs(socialAssistanceRecipientStore);
const { fetchSocialAssistancesRecipientsPaginated } =
...
const fetchData = async () => {
    await fetchSocialAssistancesRecipientsPaginated({
        ...serverOptions.value,
        ...filters.value,
    });
};
...
<template>
    ...
    <section id="List-pengajuan-Bansos" class="flex flex-col gap-[14px]">
        ...
        <CardList v-for="socialAssistanceRecipient in socialAssistancesRecipients" 
